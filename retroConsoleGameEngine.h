...
public:
	...
	jprSprite(int w, int h)
	{
		Create(w, h);
	}

	jprSprite(std::wstring sFile)
	{
		if (!Load(sFile))
			Create(8, 8);
	}
...
class retroConsoleGameEngine
{
public:
	retroConsoleGameEngine()
	{
		m_nScreenWidth = 80;
		m_nScreenHeight = 30;
	...
	void DrawSprite(int x, int y, jprSprite *sprite)
	{
		if (sprite == nullptr)
			return;
	...
	void DrawPartialSprite(int x, int y, jprSprite *sprite, int ox, int oy, int w, int h)
	{
		if (sprite == nullptr)
			return;
...
public:
	void Start()
	{
		// Start the thread
		m_bAtomActive = true;
		std::thread t = std::thread(&retroConsoleGameEngine::GameThread, this);
...
protected:
	...
	public:
		...
		jprAudioSample(std::wstring sWavFile)
		{
		...
		unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		m_AudioThread = std::thread(&retroConsoleGameEngine::AudioThread, this);
	...
	{
		((retroConsoleGameEngine*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
...
std::atomic<bool> retroConsoleGameEngine::m_bAtomActive(false);
std::condition_variable retroConsoleGameEngine::m_cvGameFinished;
std::mutex retroConsoleGameEngine::m_muxGame;
